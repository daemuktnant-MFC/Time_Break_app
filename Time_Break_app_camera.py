...
def main():
    ...
    with main_col1:
        ...
        # ---------------------------------------------------------
        # 💥💥💥 FIX: 1. ย้าย QR Scanner ขึ้นมาก่อน 💥💥💥
        # ---------------------------------------------------------
        st.markdown("หรือ สแกน QR/Barcode:") 
        scanned_id = qrcode_scanner(key="qrcode_scanner_key_new")

        # ---------------------------------------------------------
        # 💥💥💥 FIX: 2. ย้าย Logic การ Sync ID มาไว้ตรงนี้ 💥💥💥
        # ---------------------------------------------------------
        # Logic การ Sync ID จะทำงาน 'หลังจาก' scanner แต่ 'ก่อน' text_input
        
        if scanned_id:
            # ตอนนี้ทำได้แล้ว เพราะ 'manual_emp_id_input' ยังไม่ถูกวาด
        ...
        # ---------------------------------------------------------
        # 💥💥💥 FIX: 4. Sync Logic ส่วนที่เหลือ (สำหรับ Manual) 💥💥💥
        # ---------------------------------------------------------
        
        if manual_input != st.session_state.get("current_emp_id", ""):
             # ถ้ามีการพิมพ์ในกล่อง ให้ใช้ค่าที่พิมพ์
             st.session_state["current_emp_id"] = manual_input
        else:
             # ถ้าไม่มีอะไรเกิดขึ้น ให้ใช้ค่าจาก session_state (ที่อาจจะมาจากการสแกน)
